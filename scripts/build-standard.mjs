...
export function renderStandard(slug) {
  ...
  const projects = content.projects.map(project => {
    const images = project.images.map(image => {
      ...
      return `<button type="button" data-lightbox="${escape(image.src)}" aria-label="${escape(image.alt)} 확대"><img src="${escape(image.src)}" alt="${escape(image.alt)}" width="${image.width}" height="${image.height}" loading="lazy" decoding="async"></button>`;
    }).join('\n');
  ...
  ${project.cases.length ? `<details class="project-details"><summary>구현하면서 해결한 문제 ${project.cases.length}개</summary><div class="case-links">${project.cases.map(caseLink).join('')}</div></details>` : ''}
</article>`;
  }).join('\n');
...
<main class="container" id="main" tabindex="-1">
  ...
  <section class="section" id="experience" aria-labelledby="experience-title"><h2 id="experience-title">경력과 학력</h2><div class="experience-list">${content.experience.map(item => `<article class="experience-item"><h3>${escape(item.name)}</h3><p class="meta">${escape(item.period)} · ${escape(item.role)}</p>${item.description ? paragraph(item.description) : ''}</article>`).join('')}</div></section>
...
<dialog class="lightbox" id="lightbox" aria-label="프로젝트 화면 확대"><button type="button" data-lightbox-close aria-label="확대 화면 닫기">×</button><img alt=""></dialog>
<script src="script.js" defer></script>
</body>
</html>\n`;
  return { slug, content, html, css: read('src/standard.css'), script: read('src/standard.js'), assetPaths: [...assetPaths], selectedCases };
}
