...
<main class="container" id="main" tabindex="-1">
  ...
  <section class="section" id="experience" aria-labelledby="experience-title"><h2 id="experience-title">경력과 학력</h2><div class="experience-list">${content.experience.map(item => `<article class="experience-item"><h3>${escape(item.name)}</h3><p class="experience-period">${escape(item.period)}</p><p class="meta">${escape(item.role)}</p>${item.description ? paragraph(item.description) : ''}</article>`).join('')}<article class="experience-item"><h3>어학</h3><ul class="language-list">${content.languages.map(item => `<li>${escape(item)}</li>`).join('')}</ul></article></div></section>
  <section class="section" id="skills" aria-labelledby="skills-title"><h2 id="skills-title">기술 역량</h2><div class="skills-grid">${content.skills.map(skill => `<article class="skill-item"><h3>${escape(skill.title)}</h3><p class="skill-tools">${escape(skill.tools)}</p>${skill.description ? paragraph(skill.description) : ''}${skill.items?.length ? paragraph(skill.items.join(' ')) : ''}</article>`).join('')}</div></section>
