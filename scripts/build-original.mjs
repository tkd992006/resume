...
<body class="original-portfolio">
  ...
  <main id="main" tabindex="-1"><div id="top"></div>
    ...
    <section id="education" class="section education-section" aria-labelledby="education-title">${heading('Education &amp; Languages', '학력과 어학', 'education-title')}<div class="education-grid"><article><h3>${escape(education.name)}</h3>${paragraph(education.role)}${paragraph(education.period)}${education.description ? paragraph(education.description) : ''}</article><article><h3>어학</h3>${content.languages.map(paragraph).join('')}</article></div></section>
...
export function validateOriginal(output) {
  ...
  add(c.name, c.role, c.lead, ...c.about, ...c.languages, c.projectsIntro);
