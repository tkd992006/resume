...
const route = ({ initial = false, trigger } = {}) => {
  ...
  if (target instanceof HTMLDialogElement) {
    const originId = window.history.state?.dialogOrigin?.slice(1);
    const originPanel = originId ? document.getElementById(originId)?.closest('.panel') : null;
    const parentId = originPanel?.id ?? (target.classList.contains("case-modal") ? "solved-problems" : target.dataset.panel);
