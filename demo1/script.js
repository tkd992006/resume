// Direct section navigation. Cases are reachable from the first screen and
// their full narrative scrolls continuously; chronology lives in experience.
...
$$("dialog").forEach((dialog) => {
  ...
  dialog.addEventListener("close", () => {
    // Both solved-issue entries use the same route; closing restores the origin.
    if (window.location.hash === `#${dialog.id}` && dialog.matches(".sheet, .case-modal")) {
      const fallback = dialog.classList.contains("case-modal") ? "solved-problems" : dialog.dataset.panel;
      const origin = window.history.state?.dialogOrigin ?? `#${fallback ?? "who-am-i"}`;
      window.history.replaceState(null, "", origin);
      route();
    }
    const returnTo = dialogReturnFocus.get(dialog);
    if (returnTo instanceof HTMLElement && returnTo.isConnected && returnTo.getClientRects().length) returnTo.focus({ preventScroll: !returnTo.closest('dialog[open]') });
  });
});
...
const BASE_TITLE = "이상화 · 세로로 이어 읽기";
const HASH_ALIASES = { projects: "selected-work", problems: "solved-problems", top: DEFAULT_PANEL_ID, stage: DEFAULT_PANEL_ID };
...
const route = ({ initial = false, trigger } = {}) => {
  let raw;
  try { raw = decodeURIComponent(window.location.hash.replace(/^#/, "")); }
  catch { raw = ""; }
  const id = HASH_ALIASES[raw] ?? raw;
  const target = id ? document.getElementById(id) : null;
  if (!(target instanceof HTMLDialogElement)) {
    const destination = target ?? document.getElementById(DEFAULT_PANEL_ID);
    if (destination && (!initial || id)) destination.scrollIntoView({ block: "start", behavior: "instant" });
    if (destination && !initial) { destination.setAttribute("tabindex", "-1"); destination.focus({ preventScroll: true }); }
    return;
  }
  const defaultPanel = document.getElementById(DEFAULT_PANEL_ID);
  $$(".sheet[open], .case-modal[open]").forEach(dialog => {
    if (dialog !== target) dialog.close();
  });

  if (target instanceof HTMLDialogElement) {
    const parentId = target.classList.contains("case-modal") ? "solved-problems" : target.dataset.panel;
    const panel = document.getElementById(parentId ?? "") ?? defaultPanel;
    if (panel) setActivePanel(panel);
    if (!target.open) openDialog(target, trigger ?? $(`#solved-problems [data-solved-issue="${target.id}"]`) ?? $(`[data-sheet-open="${target.id}"]`) ?? undefined);
    return;
  }
  ...
  if (changed && stage) {
    stage.scrollTop = 0;
    if (getComputedStyle(stage).overflowY === "visible" && !initial) {
      $(".rail")?.scrollIntoView({ block: "start", behavior: "instant" });
    }
  }
...
document.addEventListener("click", (event) => {
  ...
  if (!(event.target instanceof Element)) return;
  const link = event.target.closest('[data-solved-issue], [data-sheet-open], a[href^="#"]');
  if (!link || link.classList.contains("skip-link")) return;
  const dialogId = link.getAttribute("data-solved-issue") ?? link.getAttribute("data-sheet-open");
  const hash = dialogId ? `#${dialogId}` : link.getAttribute("href");
  if (!hash || hash === "#") return;
  event.preventDefault();
  hideChronoTip($(".chrono-bar.is-hot"));
  if (hash !== window.location.hash) {
    const state = document.getElementById(hash.slice(1)) instanceof HTMLDialogElement
      ? { dialogOrigin: window.location.hash || `#${DEFAULT_PANEL_ID}` } : null;
...
const readingObserver = new IntersectionObserver(entries => {
      for (const entry of entries) if (entry.isIntersecting) {
        railItems.forEach(item => { const active = item.dataset.panel === entry.target.id; item.classList.toggle('is-active', active); const link = item.querySelector('a'); if (active) link.setAttribute('aria-current', 'location'); else link.removeAttribute('aria-current'); });
      }
    }, { rootMargin: '-10% 0px -65% 0px' });
    panels.forEach(panel => readingObserver.observe(panel));
